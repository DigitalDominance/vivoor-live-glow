...
-- Policies for vods
CREATE POLICY IF NOT EXISTS "VODs are viewable by everyone"
ON public.vods FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Users can insert their own VODs"
ON public.vods FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own VODs"
ON public.vods FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own VODs"
...
-- Policies for clips
CREATE POLICY IF NOT EXISTS "Clips are viewable by everyone"
ON public.clips FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Users can insert their own clips"
ON public.clips FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own clips"
ON public.clips FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own clips"
