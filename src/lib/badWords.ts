...
/**
 * Cleans inappropriate language from text with blur effect
...
/**
 * Processes text to blur bad words for chat display
 * @param text - The text to process
 * @returns HTML string with bad words wrapped in blur spans
 */
export function blurBadWords(text: string): string {
  if (!text || typeof text !== 'string') return text;
  
  let processedText = text;
  badWordsList.forEach(badWord => {
    const regex = new RegExp(`\\b(${badWord})\\b`, 'gi');
    processedText = processedText.replace(regex, '<span class="blur-sm hover:blur-none transition-all duration-300 cursor-pointer" title="Click to reveal">$1</span>');
  });
