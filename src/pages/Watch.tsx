          ...
          <ChatPanel
            messages={[
              ...chatMessages.filter(msg => {
                // Only filter out confirmed messages that match current optimistic messages
                const hasOptimistic = optimisticMessages.some(opt => 
                  opt.text === msg.text && 
                  msg.user.id === (currentUserProfile?.id || identity?.id)
                );
